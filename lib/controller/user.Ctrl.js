...
module.exports = {
    ...
    AuthenticateUser: function(req,res)
    {
        ...
        connection.acquire(function (err, con) {
            ...
            // console.log(encpass);
            con.query("SELECT id,name,firstlogin,role,locationid,customerid from users WHERE mobile =? AND password = ?", [req.body.mobile, encpass], function (err, result) {
                if (err) {
                    console.log('---1',err)
                    res.send({
                        success: false,
                    ...
                    if (result.length == 1) {
                        if (result[0].role === 'Superadmin') {
                            ...
                            var sql = "SELECT approval from location WHERE id = " + result[0].locationid;
                        }
                        con.query(sql, function (err, locationapproval) {
                            if (err) {
                                console.log('---2',err)
                                res.send({
                                    success: false,
                                ...
                                if (result[0].role === 'Superadmin') {
                                    ...
                                    if (locationapproval[0].approval === 1) {
                                        res.send({
                                            success: false,
                                            type: "error",
                                            title: "Oops!",
    ...
    AuthenticateUserAuto: function(req,res)
    {
        connection.acquire(function (err, con) {
           ...
           // var encpass = cryptconf.encrypt(req.body.password)
            ...
            // console.log(encpass);
            con.query("SELECT id,name,firstlogin,role,locationid,customerid from users WHERE locationid =? AND id = ?", [req.body.locationid, req.body.id], function (err, result) {
                if (err) {
                    console.log('---1',err)
                    res.send({
                        success: false,
                    ...
                    if (result.length == 1) {
                        if (result[0].role === 'Superadmin') {
                            ...
                            var sql = "SELECT approval from company WHERE id = " + result[0].locationid;
                        }
    ...
    VerifyCompanyEmail: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                if(req.body.id)
                {
                    var appdenquery = ' AND locationid != '+req.body.id+'';
                }
                else
                {
                    var appdenquery = ''
                ...
                if (!req.body.CompanyDetails.id) {
                    con.query("INSERT INTO `company` set ?", req.body.CompanyDetails, function (err, result) {
                        if (err) {
                            ...
                            var sql2 = 'INSERT INTO `users`(`name`, `email`, `mobile`, `role`, `password`, `createdby`, `locationid`) VALUES (?,?,?,"Admin","'+password+'",'+req.decoded.logedinuser.id+', '+result.insertId+')';
                ...
                if (req.body.CompanyDetails.id) {
                    ...
                    con.query("UPDATE `company` set ? WHERE id = ?", [req.body.CompanyDetails, req.body.CompanyDetails.id], function (err, result) {
                        if (err) {
                            ...
                            var sql2 = 'UPDATE `users` SET `name`=?,`email`=?,`mobile`=? WHERE  `locationid`= ? AND role = "Admin" ORDER BY id ASC LIMIT 1';
    ...
    deleteCompanyDetails: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                con.query('UPDATE `company` set status= (CASE WHEN status = 0 THEN 1 ELSE 0 END) WHERE id = '+req.params.locationid, function(err,result){
                    if(err)
                {
                    res.send({
                        status: 1,
    ...
    getCompanyDetails: function(req, res)
    {
        if (req.decoded.success == true) {   
            connection.acquire(function(err, con){
                ...
                con.query('SELECT * FROM `company` WHERE `id` = '+req.decoded.logedinuser.locationid, function(err,result){
                    if(err)
                {
                    res.send({
                        status: 1,
